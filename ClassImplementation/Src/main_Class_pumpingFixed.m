function [inputs, outputs, optimDetails, processedOutputs, simulationQSM] = main_Class_pumpingFixed(inputs, loggerID)
  ...
  %   loggerID (string): Logger ID label, used inside QSM class
  %
  % Outputs:
  ...
  if inputs.windSpeedReference(1)>inputs.windSpeedReference(end)
        ...
        0.1 * inputs.Cl_eff_F .* nx];     % reel-out C_L
  else
        inputs.x0 = [stroke_length(2), ...                     % stroke length
        deg2rad(elevationAngle_deg(2)), ...                % pattern elevation
        deg2rad(coneAngle_deg(2)), ...                     % cone angle
